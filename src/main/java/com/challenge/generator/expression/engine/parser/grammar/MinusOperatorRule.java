...
/**
 * Provides parsing support for the minus operator in arithmetic expressions.
 */
@Component
public class MinusOperatorRule implements GrammarRuleProvider {
    @Override
    ...
    public TokenParser getTokenParser() {
        return new TokenParser() {
            ...
            public ExpressionNode parseInfix(ExpressionParser parser, TokenStream stream, ExpressionNode left, Token token) {
                ...
                return new BinaryOpNode(Operator.MINUS.getSymbol(), left, right, token.position());
            }
