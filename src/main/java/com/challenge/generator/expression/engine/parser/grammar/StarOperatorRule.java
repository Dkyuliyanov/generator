...
/**
 * Provides parsing support for the multiplication operator in arithmetic expressions.
 */
@Component
public class StarOperatorRule implements GrammarRuleProvider {
    @Override
    ...
    public TokenParser getTokenParser() {
        return new TokenParser() {
            ...
            public ExpressionNode parseInfix(ExpressionParser parser, TokenStream stream, ExpressionNode left, Token token) {
                ...
                return new BinaryOpNode(Operator.MULTIPLY.getSymbol(), left, right, token.position());
            }
